...
cd /campaign/VB-FM5HPC-001/Vilte/Projects/BrainAgeGWAS_2024/genetic-corr/output/

# output header
echo -e "Trait\tGeneticCorrelation\tSE\tZscore\tPvalue" > results.tsv

# loop through each file
for file in brainageFactor-cor-*.log; do
  # Extract trait name (e.g., adhd, asd, etc.)
  trait=$(echo "$file" | sed -E 's/brainageFactor-cor-(.*)\.log/\1/')
