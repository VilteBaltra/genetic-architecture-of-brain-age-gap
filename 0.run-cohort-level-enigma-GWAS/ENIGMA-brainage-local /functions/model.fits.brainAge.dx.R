...
model.fits.brainAge.dx=function(model, brain_age_output){
    ...
    variable$DX[variable$DX==1] <- "cases"
    ...
    ##Make a plot of MAE distribution per Dx
      ...
      p00 <- ggplot(variable, aes(y=AE, x=DX)) + geom_boxplot(alpha=0.5)+ geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
    ...
    p02 <- ggplot(variable, aes(y=AE, x=agegroups, fill=DX)) + geom_boxplot(alpha=0.5)+ geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
    ...
    ##Make a plot of devAge distribution per Dx
       variable$DX=factor(variable$DX)
       p03 <- ggplot(variable, aes(y=devAge, x=DX)) + geom_boxplot(alpha=0.5)+ geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
    ...
    variable$agegroups=factor(variable$agegroups)
    p04 <- ggplot(variable, aes(y=devAge, x=agegroups, fill=DX)) + geom_boxplot(alpha=0.5)+ geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
