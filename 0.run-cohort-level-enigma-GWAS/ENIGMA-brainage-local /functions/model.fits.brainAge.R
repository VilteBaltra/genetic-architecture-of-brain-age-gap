...
#   # 
#   # 
#   # output_males <- read.csv("males_cases_raw_out.csv", header=T, sep="\t")
#   # males_cases$predAge <- output_males$age_prediction
#   # 
#   # output_females <- read.csv("females_cases_raw_out.csv", header=T, sep="\t")
#   # females_cases$predAge <- output_females$age_prediction
#   # 
...
#   ##variable <- males
#   ##variable <- females
#   ##variable <- males_cases
#   ##variable <- females_cases
#   ##variable <- males_hc
#   variable <- females_hc
#   
model.fits.brainAge=function(model, brain_age_output){
  ...
  if ("SITE" %in% names(variable)){
    ...
    p00 <- ggplot(variable, aes(y=AE, x=SITE)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
    ...
    p00 <- ggplot(variable, aes(y=AE, x=1)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
  ...
  p02 <- ggplot(variable, aes(y=AE, x=agegroups)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
  ...
  if ("SITE" %in% names(variable)){
    variable$SITE=factor(variable$SITE)
    p03 <- ggplot(variable, aes(y=devAge, x=SITE)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
    
    p03 <- p03 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                    panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
  } else {
    p03 <- ggplot(variable, aes(y=devAge, x=1)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
  ...
  variable$agegroups=factor(variable$agegroups)
  p04 <- ggplot(variable, aes(y=devAge, x=agegroups)) + geom_boxplot(alpha=0.5)+  geom_smooth(formula = y ~ x, method = "loess") + labs(x = "", y = "")
  ...
  if ("SITE" %in% names(variable)){
    ...
    p1 <- ggplot(variable, aes(x=predAge, y=AGE, color=SITE))+ geom_point(alpha=0.5)+
      ...
      scale_colour_hue(l=50)+ geom_smooth(formula = y ~ x, method = lm, colour="black", size=0.5, se=TRUE, fullrange=TRUE)+
